...
// New Route: Save Notification
router.post('/Save_Notifications', authMiddleware, async (req, res) => {
  try {
    const { title, message, currency, amount } = req.body;
    const userId = req.user.userId;

    if (!title || !message || !currency || !amount) {
      return res.status(400).json({
        status: 400,
        message: 'Missing required fields: title, message, currency, amount',
      });
    }
...
router.get('/Fetch_Notifications', authMiddleware, async (req, res) => {
  try {
    const userId = req.user.userId;
    const notifications = await Notification.find({ userId })
      .sort({ createdAt: -1 })
      .select('title message currency amount createdAt read');
...
router.patch('/MarkReadAsnotifications/:id/read', authMiddleware, async (req, res) => {
  try {
    const userId = req.user.userId;
